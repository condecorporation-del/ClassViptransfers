...
export function MarketingTab() {
  ...
  if (error) {
    return (
      <div className="flex items-start gap-3 rounded-2xl border border-red-200/70 bg-red-50/60 p-6 text-red-700">
        ...
        {kpis.map((kpi, index) => (
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <p className="text-[9px] font-black uppercase tracking-[0.22em] leading-tight text-muted-foreground">
                {kpi.label}
              </p>
      ...
      >
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm font-bold text-foreground">Reservaciones por dia - Ultimos 30 dias</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        >
          <div className="mb-5">
            ...
            <p className="text-sm font-bold text-foreground">Reservaciones por zona</p>
          </div>
          {zoneData.length === 0 ? (
            <div className="flex h-56 items-center justify-center text-sm text-muted-foreground">Sin datos</div>
          ) : (
            <ResponsiveContainer width="100%" height={240}>
        ...
        >
          <div className="mb-5">
            ...
            <p className="text-sm font-bold text-foreground">Confirmadas vs pendientes - 3 meses</p>
          </div>
          ...
          <div className="mt-4 flex items-center gap-5">
            {[
              ...
              <div key={legend.label} className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-muted-foreground">{legend.label}</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      >
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm font-bold text-foreground">Tripadvisor, Google, Facebook e Instagram</p>
          </div>
          <button
            type="button"
            onClick={() => void fetchData()}
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {socialReputationCards.map((card) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="group rounded-2xl border border-border/70 bg-background p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:border-gold/30 hover:shadow-md"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">{card.platform}</p>
                  <p className="mt-1 text-sm font-semibold text-foreground">{card.handle}</p>
                </div>
                <ExternalLink size={14} className="text-muted-foreground transition-colors group-hover:text-gold" />
              </div>

              <div className="mt-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.1em] text-muted-foreground">{card.metricLabel}</p>
