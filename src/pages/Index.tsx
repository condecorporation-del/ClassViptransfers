    ...
    <div className="overflow-hidden">
      ...
      <section className="relative h-screen min-h-[700px] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center px-4 text-center">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h1 className="font-display text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold mb-6 max-w-5xl text-off-white text-luxury-shadow leading-[1.1]">
                {t(heroTexts[currentImage].titleKey)}
              </h1>
              <p className="text-off-white/60 text-lg md:text-xl max-w-2xl mx-auto mb-10 font-light font-accent">
          ...
          >
            ...
            <a
              ...
              className="border border-off-white/20 bg-off-white/5 backdrop-blur-md px-10 py-4 rounded-full font-semibold text-base text-off-white hover:bg-off-white/10 hover:border-gold/30 transition-all flex items-center gap-3"
            >
          ...
          {heroImages.map((_, i) => (
            <button
              ...
              className={`h-1 rounded-full transition-all duration-700 ${
                i === currentImage ? 'w-12 bg-gold' : 'w-4 bg-off-white/30 hover:bg-off-white/50'
              }`}
            />
          ))}
        </div>
        ...
        <motion.div
          ...
          className="absolute bottom-8 left-1/2 -translate-x-1/2 text-off-white/30 z-10"
        >
          <ChevronDown size={28} />
        </motion.div>
      </section>
            ...
            {stats.map((stat, i) => (
              <div key={i} className="text-center">
                <div className="flex items-center justify-center gap-1.5 mb-1">
                  <span className="font-display text-3xl md:text-4xl font-bold text-gold stat-glow">{stat.value}</span>
                  {stat.icon}
                </div>
      ...
      <section className="py-24 px-4">
        <div className="container mx-auto max-w-6xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
            ...
            <h2 className="font-display text-3xl md:text-5xl lg:text-6xl font-bold mb-5">{t('home.transfers.title')}</h2>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
              ...
              <div className="flex items-center gap-4 mb-5">
                ...
                <div>
                  <h3 className="font-display text-2xl font-bold">{t('home.transfers.private')}</h3>
                  <p className="text-muted-foreground text-sm">{t('home.transfers.privateDesc')}</p>
                </div>
              </div>
              <div className="space-y-3 mb-6">
                <div className="flex items-center justify-between bg-accent/60 rounded-xl px-5 py-4 border border-border/50">
                  <div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {includes.slice(0, 4).map((inc, i) => (
                  <span key={i} className="text-[11px] text-muted-foreground bg-accent/50 px-3 py-1 rounded-full flex items-center gap-1">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ delay: 0.15 }}
              ...
              <div className="flex items-center gap-4 mb-5">
                ...
                <div>
                  <h3 className="font-display text-2xl font-bold">{t('home.transfers.shuttle')}</h3>
              ...
              <Link to="/book" className="border border-gold/30 text-gold px-8 py-3.5 rounded-full text-sm font-bold inline-flex items-center gap-2 hover:bg-gold/10 transition-all group">
                {t('home.transfers.bookThis')} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
              </Link>
            </motion.div>
          </div>
      ...
      <section className="py-24 px-4">
        <div className="container mx-auto max-w-6xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-12">
            ...
            <h2 className="font-display text-3xl md:text-5xl lg:text-6xl font-bold mb-5">{t('home.activities.title')}</h2>
          ...
          >
            {activities.map((act, i) => (
              <motion.div key={i} variants={fadeUp}
                ...
                <p className="font-display text-lg font-bold mb-2">{t(act.key)}</p>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid sm:grid-cols-2 gap-6 mb-8">
            <motion.div variants={fadeUp} className="glass-card rounded-2xl p-8 premium-card border border-border text-center">
              ...
              <p className="font-display text-2xl font-bold mb-2">{t('home.activities.combo')}</p>
              <p className="text-muted-foreground text-sm mb-3">{t('home.activities.comboActivities')}</p>
              <p className="text-gold text-3xl font-bold mb-4 stat-glow">{t('home.activities.comboPrice')}</p>
            ...
            <motion.div variants={fadeUp} className="glass-card rounded-2xl p-8 premium-card border-2 border-gold/30 text-center relative overflow-hidden">
              ...
              <p className="font-display text-2xl font-bold mb-2">{t('home.activities.crazyCombo')}</p>
              <p className="text-muted-foreground text-sm mb-3">{t('home.activities.crazyActivities')}</p>
              <p className="text-gold text-3xl font-bold mb-4 stat-glow">{t('home.activities.crazyPrice')}</p>
              <Link to="/book-activities" className="gold-gradient text-secondary-foreground px-8 py-3 rounded-full text-sm font-bold inline-flex items-center gap-2 hover:brightness-110 transition-all gold-glow">
      ...
      <section className="py-24 px-4">
        <div className="container mx-auto max-w-5xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
            ...
            <h2 className="font-display text-3xl md:text-5xl lg:text-6xl font-bold mb-5">{t('howItWorks.title')}</h2>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid md:grid-cols-3 gap-12">
            {steps.map((step, i) => (
              <motion.div key={i} variants={fadeUp} className="text-center relative">
                ...
                <h4 className="font-display text-xl font-bold mb-3">{t(step.titleKey)}</h4>
      ...
      <section className="py-24 px-4">
        <div className="container mx-auto max-w-5xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
            ...
            <h2 className="font-display text-3xl md:text-5xl lg:text-6xl font-bold mb-5">{t('testimonials.title')}</h2>
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid md:grid-cols-3 gap-6">
            {testimonials.map((test, i) => (
              <motion.div key={i} variants={fadeUp}
                ...
                <p className="text-foreground/80 text-sm mb-6 italic leading-relaxed font-accent text-lg">"{t(test.key)}"</p>
      ...
      <section className="py-24 px-4">
        <div className="container mx-auto max-w-3xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
            <h2 className="font-display text-3xl md:text-5xl font-bold mb-5">{t('faq.title')}</h2>
          </motion.div>
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto max-w-4xl">
          <motion.div
            ...
            viewport={{ once: true }}
            variants={fadeUp}
            className="glass-card rounded-3xl p-12 md:p-16 text-center gradient-border relative overflow-hidden"
          >
            <div className="absolute inset-0 shimmer pointer-events-none" />
            <h2 className="font-display text-3xl md:text-5xl font-bold mb-4">{t('cta.title')}</h2>
            <p className="text-muted-foreground text-lg font-light max-w-lg mx-auto mb-8">{t('cta.subtitle')}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="border border-gold/30 text-gold px-10 py-4 rounded-full font-semibold text-base hover:bg-gold/10 transition-all flex items-center gap-3"
              >
                <MessageCircle size={18} /> {t('hero.cta2')}
              </a>
            </div>
