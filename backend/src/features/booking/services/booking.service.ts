...
import { moneyToCents, CreateBookingInput, ListBookingsInput, UpdateBookingInput } from '../../../shared/lib/validation';
import { BookingStatus, BookingType, BookingSource, Prisma } from '@prisma/client';
...
export class BookingService {
  ...
  async createDraftBooking(input: CreateBookingInput, source: BookingSource = 'WEBSITE') {
    ...
    if (input.type === 'TRANSPORTATION' && input.areaId && input.tripType) {
      try {
        ...
        const message = getErrorMessage(error, 'Invalid area for transport pricing');
        console.error('[Booking] Area pricing failed:', message);
      ...
      try {
        ...
        const message = getErrorMessage(error, 'Invalid zones or pricing data');
        console.error('[Booking] Pricing calculation failed (legacy path):', message);
        throw new Error(
          `Pricing calculation failed: ${message}. Please check route and try again.`
        );
      }
    } else {
   ...
   */
  async listBookings(filters: ListBookingsInput) {
  ...
  async exportBookingsToCSV(date: string): Promise<string> {
    ...
    const rows = bookings.map(booking => {
      ...
      const paymentReference = payment?.orderId || '';
  ...
  async updateBooking(bookingId: string, data: UpdateBookingInput, userId?: string, userEmail?: string) {
    ...
    if (data.flightNumber !== undefined) update.flightNumber = data.flightNumber;
    if (data.arrivalTime !== undefined) update.arrivalTime = data.arrivalTime;
    if (data.departureFlightNumber !== undefined) {
      update.departureFlightNumber = data.departureFlightNumber;
    }
    if (data.departureTime !== undefined) update.departureTime = data.departureTime;
    if (data.pickupLocation !== undefined) update.pickupLocation = data.pickupLocation;
    if (data.dropoffLocation !== undefined) update.dropoffLocation = data.dropoffLocation;
