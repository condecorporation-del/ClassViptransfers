...
import { CreateBookingInput } from '../../../shared/lib/validation';
import { getKnowledgeForPrompt, AI_KNOWLEDGE } from '../lib/ai-knowledge';
import { BookingType, BookingStatus, BookingSource, Prisma } from '@prisma/client';
...
const responseCache = new Map<string, { data: CachedChatResult; expiresAt: number }>();
...
function getCached(key: string): CachedChatResult | null {
...
function setCache(key: string, data: CachedChatResult): void {
...
function getHttpStatus(error: unknown): number | undefined {
  ...
  const candidate = error as OpenAIStatusError;
...
Respond ONLY with this JSON:
{
  ...
  async chat(
    ...
    if (cached) {
      ...
      await prisma.aIConversation.create({
        data: {
          ...
          extractedData: toPrismaJson(cached.extracted),
    ...
    if (history.length === 0 && simpleKey) {
      ...
      await prisma.aIConversation.create({
        data: {
          ...
          extractedData: toPrismaJson(extracted),
          missingFields: ['date', 'passengers', 'pickup', 'dropoff'],
          nextAction: 'ask_more',
          messageType: 'text',
        },
      });
    ...
    try {
      ...
      try {
        ...
        console.warn('[AI Chat] JSON parse fallback:', getErrorMessage(parseErr, 'Invalid JSON'));
        extracted = this.parseExtractionFromText(responseContent);
        reply = typeof responseContent === 'string' ? responseContent : this.generateReply(extracted, locale);
      }
      this.ensureExtractedShape(extracted);
      ...
      console.error('[AI Chat] Service error:', msg);
  ...
  private extractedToBookingInput(
    ...
    return {
      type: (extracted.intent?.toUpperCase() || 'TRANSPORTATION') as CreateBookingInput['type'],
      customer: {
