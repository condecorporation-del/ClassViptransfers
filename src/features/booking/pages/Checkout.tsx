...
function StripePaymentForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentIntent?.status === 'succeeded') {
      try {
        const response = await fetch(`${getApiBaseUrl()}/api/stripe/confirm-payment`, {
          method: 'POST',
...
export default function Checkout() {
  ...
  const getBookingToken = useCallback(() => {
    if (!bookingId) return '';
    return bookingToken ? `?token=${bookingToken}` : '';
  ...
  const loadBookingAndIntent = useCallback(async () => {
    ...
    try {
      ...
      if (!bookingRes.ok) throw new Error(checkoutCopy.loadFailure[normalizedLang]);
  ...
  }, [bookingId, bookingToken, getBookingToken, normalizedLang]);
